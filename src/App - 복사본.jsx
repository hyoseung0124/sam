...
function App() {
  ...
  const supportsClientFileAccess = () => Boolean(window.showDirectoryPicker) && window.isSecureContext
  ...
  const onUnlock = async () => {
    ...
    if (supportsClientFileAccess()) {
      ...
      setAuthError('현재 접속 방식에서는 로컬 파일 접근이 제한됩니다. HTTPS(또는 localhost)로 접속해 주세요.')
      return
    }
  ...
  const onDeleteSelected = () => {
    ...
    const confirmed = window.confirm(
      `[삭제 확인]\n선택된 ${deleteCount}건을 정말 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다.`,
    )
    if (!confirmed) return
        ...
        <section className="auth-card">
          ...
          <section className="auth-guide" aria-label="마스터키 인증 안내">
            ...
            <ul>
              ...
              <li>로컬 파일 접근을 위해 HTTPS 또는 localhost 환경이 필요합니다.(Edge, Chrome 사용, Safari 사용불가)</li>
            </ul>
          </section>
          <p></p>
