...
:HTTP_MODE
cd /d "%SCRIPT_DIR%"
echo [%date% %time%] Starting server-access-manager...>>"%LOG_FILE%"
...
where npm >nul 2>&1
if not errorlevel 1 (
	echo [%date% %time%] Building latest source...>>"%LOG_FILE%"
	call npm run build >>"%LOG_FILE%" 2>&1
	if errorlevel 1 (
		echo [%date% %time%] ERROR: Build failed before server start.>>"%LOG_FILE%"
		echo Build failed. Check server-access-manager-startup.log
		exit /b 1
	)
) else (
	echo [%date% %time%] WARNING: npm not found. Skip build and run existing dist.>>"%LOG_FILE%"
)
...
REM "%NODE_EXE%" "%SERVER_SCRIPT%" --host 0.0.0.0 --port 4173 >>"%LOG_FILE%" 2>&1
"%NODE_EXE%" "%SERVER_SCRIPT%" --host 127.0.0.1 --port 4173 >>"%LOG_FILE%" 2>&1
...
:HTTPS_MODE
cd /d "%SCRIPT_DIR%"
echo [%date% %time%] Starting server-access-manager HTTPS mode...>>"%LOG_FILE%"
...
if not errorlevel 1 (
	...
	call npm run build >>"%LOG_FILE%" 2>&1
	if errorlevel 1 (
		echo [%date% %time%] ERROR: Build failed before HTTPS server start.>>"%LOG_FILE%"
		echo Build failed. Check server-access-manager-startup.log
		exit /b 1
	)
) else (
...
"%NODE_EXE%" "%SERVER_SCRIPT%" --https --host 0.0.0.0 --port 4173 --pfx "%HTTPS_PFX%" --pfx-pass "%PFX_PASS%" >>"%LOG_FILE%" 2>&1
set "EXIT_CODE=%ERRORLEVEL%"
...
where npm >nul 2>&1
if errorlevel 1 (
	echo npm.exe not found. Install Node.js/npm to use dev mode.
	exit /b 1
)

echo Starting Vite dev mode with hot reload at http://0.0.0.0:4173
